...
describe("Ghost reminder bug (issue #13317)", () => {
  ...
  it("coalesced owner continuation preserves an explicit target none policy", async () => {
    await withTempHeartbeatSandbox(async ({ tmpDir, storePath, replySpy }) => {
      const { cfg, sessionKey } = await createConfig({
        tmpDir,
        storePath,
      ...
      const backgroundEvent = "[FlowOS Execution] child completed; validate and complete it";
      const execEvent = "Exec completed (none-run, code 0) :: deploy succeeded";
      const cronEvent = "Cron: rotate the maintenance snapshot";
      enqueueSystemEvent(backgroundEvent, {
        sessionKey,
        contextKey: "flowos-execution:execution-1:attempt-1:ended",
      });
      enqueueSystemEvent(execEvent, { sessionKey, contextKey: "exec:none-run" });
      enqueueSystemEvent(cronEvent, { sessionKey, contextKey: "cron:maintenance" });
      ...
      try {
        ...
        expect(dispatched.every((wake) => wake.heartbeat === undefined)).toBe(true);
